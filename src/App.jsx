...
const getCategoria = (nome, darkMode) => {
  ...
  if (n.includes('nubank') || n.includes('fatura')) 
      return { label: 'FINANCEIRO', icon: <PiggyBank size={18} />, color: 'bg-purple-500/10 text-purple-400 border-transparent', banco: n.includes('nubank') ? 'Nubank' : 'Nubank' };
    ...
    <div className={`min-h-screen p-4 md:p-10 transition-all duration-700 ${darkMode ? 'bg-[#020617] text-gray-100' : 'bg-[#f8f9ff] text-gray-900'}`}>
      ...
      <div className="max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-center gap-6 mb-12">
        <h1 className="text-4xl font-black tracking-tighter flex items-center gap-4 text-left w-full">
          ...
          <button onClick={() => setDarkMode(!darkMode)} className="p-3 rounded-2xl bg-white/5 hover:bg-white/10 hover:scale-110 active:scale-95 transition-all">
            {darkMode ? <Sun size={20} className="text-yellow-400"/> : <Moon size={20}/>}
          </button>
        </h1>
        <div className="flex gap-3 w-full md:w-auto">
          <button onClick={() => setIsModalOpen(true)} className="flex-1 md:flex-none bg-[#5643ff] hover:bg-[#4532ff] text-white px-10 py-4 rounded-2xl font-black uppercase text-[11px] tracking-widest shadow-lg shadow-[#5643ff]/20 hover:scale-105 active:scale-95 transition-all">
            <Plus size={18} className="inline mr-2" /> Novo Registro
          </button>
          <button onClick={() => signOut(auth)} className="p-4 rounded-2xl border border-white/5 hover:bg-red-500/10 hover:text-red-500 hover:scale-110 transition-all"><LogOut size={20} /></button>
        </div>
      </div>

      {/* CARDS */}
      <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-4 gap-6 mb-12">
        <div className={`p-8 rounded-[40px] border transition-all hover:scale-[1.02] ${darkMode ? 'bg-white/[0.03] border-white/5' : 'bg-white border-gray-100'}`}>
          <div className="text-indigo-400 font-black uppercase text-[10px] mb-3 tracking-widest text-left">Saldo Atual</div>
        ...
        <div className={`p-8 rounded-[40px] border transition-all hover:scale-[1.02] ${darkMode ? 'bg-white/[0.03] border-white/5' : 'bg-white border-gray-100'}`}>
          <div className="text-red-500 font-black uppercase text-[10px] mb-3 tracking-widest text-left">Pendente</div>
        ...
        <div className={`p-8 rounded-[40px] border transition-all hover:scale-[1.02] ${darkMode ? 'bg-white/[0.03] border-white/5' : 'bg-white border-gray-100'}`}>
          <div className="text-emerald-500 font-black uppercase text-[10px] mb-3 tracking-widest text-left">Pago</div>
      ...
      {/* ÁREA DE FILTROS COM ANIMAÇÕES RECUPERADAS */}
      <div className={`max-w-7xl mx-auto rounded-[45px] shadow-2xl border ${darkMode ? 'bg-gray-900/40 border-white/5' : 'bg-white border-gray-50'}`}>
        <div className="p-10 space-y-8">
          <div className="flex flex-col xl:flex-row justify-between items-center gap-6">
            <div className={`flex items-center gap-4 px-6 py-4 w-full xl:max-w-md rounded-2xl focus-within:ring-2 focus-within:ring-[#5643ff]/50 transition-all ${darkMode ? 'bg-white/5' : 'bg-gray-50'}`}>
            ...
            {/* BOTÕES DE BANCO COM ANIMAÇÃO */}
            <div className="flex flex-wrap justify-center gap-2">
              {["Todos", "BB", "C6", "Nubank"].map((b) => (
                <button key={b} onClick={() => setFiltroBanco(b)} 
                  className={`px-5 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all border hover:scale-110 active:scale-95 ${
                    filtroBanco === b 
                    ? 'bg-white/10 border-indigo-500 text-white shadow-lg shadow-indigo-500/30' 
            ...
            {/* ABAS DE STATUS COM ANIMAÇÃO */}
            <div className={`flex p-1.5 rounded-2xl ${darkMode ? 'bg-white/5' : 'bg-gray-100'}`}>
              {["Pendentes", "Pagos", "Todos"].map((aba) => (
                <button key={aba} onClick={() => setFiltroStatus(aba)} 
                  className={`px-8 py-3 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all hover:scale-105 active:scale-95 ${
                    filtroStatus === aba 
                    ? 'bg-[#5643ff] text-white shadow-lg shadow-[#5643ff]/40' 
            ...
            <table className="w-full">
              <thead>
                <tr className="text-[10px] font-black uppercase tracking-widest text-gray-500 border-b border-white/[0.02]">
                  <th className="px-6 py-6 text-center italic">SEL.</th>
                  <th className="px-8 py-6 text-left italic">Vencimento</th>
                  <th className="px-8 py-6 text-left italic">Descrição</th>
                  <th className="px-8 py-6 text-left italic">Valor</th>
                  <th className="px-8 py-6 text-left italic">Status</th>
                  <th className="px-8 py-6 text-right italic">Ações</th>
                </tr>
              </thead>
              <tbody className="text-sm font-bold">
                {filteredData.map((item) => {
                  ...
                  return (
                    <tr key={item.id} className={`transition-all duration-300 border-b border-white/[0.02] ${isSelected ? 'bg-[#5643ff]/10' : vencido ? 'bg-red-500/[0.04] animate-pulse' : ''} hover:bg-white/[0.02]`}>
                      <td className="px-6 py-6 text-center">
                        <button onClick={() => setSelectedIds(prev => prev.includes(item.id) ? prev.filter(i => i !== item.id) : [...prev, item.id])} 
                          className={`w-6 h-6 rounded-md border-2 flex items-center justify-center mx-auto transition-all hover:scale-110 active:scale-90 ${isSelected ? 'bg-[#5643ff] border-[#5643ff] shadow-[0_0_10px_rgba(86,67,255,0.4)]' : 'border-gray-600'}`}>
                          {isSelected && <X size={14} className="text-white" strokeWidth={4} />}
                        </button>
                      </td>
                      <td className="px-8 py-6 text-left">
                        <div className="flex flex-col">
                          <span className={vencido ? "text-red-500 font-black underline decoration-red-500/50" : "text-gray-400"}>
                            {item.data ? new Date(item.data + "T12:00:00").toLocaleDateString('pt-BR') : "--/--"}
                          </span>
                          {vencido && <span className="text-[8px] font-black text-red-500 flex items-center gap-1 mt-1 uppercase tracking-tighter animate-bounce"><AlertCircle size={10}/> VENCIDO</span>}
                        </div>
                      </td>
                      <td className="px-8 py-6 text-left">
                        <div className="flex items-center gap-4 group">
                          <span className={`p-2.5 rounded-xl transition-transform group-hover:scale-110 ${cat.color}`}>{cat.icon}</span>
                          <div className="flex flex-col">
                      ...
                      <td className="px-8 py-6 text-left">
                        <select className={`px-4 py-2 rounded-full text-[10px] font-black uppercase outline-none transition-all hover:scale-105 cursor-pointer ${item.status === 'Confirmada' ? 'bg-emerald-500/10 text-emerald-500' : 'bg-amber-500/10 text-amber-500'}`} 
                      ...
                      <td className="px-8 py-6 text-right">
                        <div className="flex justify-end gap-5 opacity-40 hover:opacity-100 transition-all">
                          <button onClick={() => { setEditingId(item.id); setFormData({...item}); setIsModalOpen(true); }} className="hover:text-indigo-500 hover:scale-125"><Edit3 size={18}/></button>
                          <button onClick={async () => { if(window.confirm("Excluir?")) await deleteDoc(doc(db, "registros", item.id)) }} className="hover:text-red-500 hover:scale-125"><Trash2 size={18}/></button>
                        </div>
                      </td>
                    </tr>
                  );
      ...
      {selectedIds.length > 0 && (
        <div className="fixed bottom-10 left-1/2 -translate-x-1/2 z-50 animate-in slide-in-from-bottom-10 duration-500">
          <div className="bg-gray-900 border border-[#5643ff]/50 px-10 py-5 rounded-[30px] shadow-[0_0_30px_rgba(86,67,255,0.4)] flex items-center gap-8 backdrop-blur-xl hover:scale-105 transition-transform">
            ...
            <button onClick={() => setSelectedIds([])} className="text-gray-500 hover:text-white hover:scale-125 transition-all"><X size={20}/></button>
          </div>
        </div>
      )}
      ...
      {/* MODAL */}
      {isModalOpen && (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-md flex items-center justify-center p-6 z-[60] animate-in fade-in duration-300">
          <div className={`w-full max-w-xl rounded-[50px] p-12 border transform animate-in zoom-in-95 duration-300 ${darkMode ? 'bg-gray-900 border-white/10 text-white' : 'bg-white border-transparent text-gray-900'}`}>
            <div className="flex justify-between items-center mb-8"><h2 className="text-3xl font-black italic tracking-tighter text-left">Novo Registro</h2><button onClick={closeModal} className="hover:rotate-90 transition-all"><X size={24} /></button></div>
            <form onSubmit={handleSubmit} className="space-y-8">
              <input required className={`w-full p-5 rounded-2xl outline-none font-bold text-lg border-2 border-transparent focus:border-[#5643ff] transition-all ${darkMode ? 'bg-white/5' : 'bg-gray-50'}`} value={formData.nome} onChange={(e) => setFormData({...formData, nome: e.target.value})} placeholder="Descrição" />
              <div className="grid grid-cols-2 gap-6">
                <input required type="number" step="0.01" className={`w-full p-5 rounded-2xl outline-none font-bold text-lg border-2 border-transparent focus:border-[#5643ff] transition-all ${darkMode ? 'bg-white/5' : 'bg-gray-50'}`} value={formData.valor} onChange={(e) => setFormData({...formData, valor: e.target.value})} placeholder="Valor R$" />
                <input type="date" className={`w-full p-5 rounded-2xl outline-none font-bold text-lg border-2 border-transparent focus:border-[#5643ff] transition-all ${darkMode ? 'bg-white/5' : 'bg-gray-50'}`} value={formData.data} onChange={(e) => setFormData({...formData, data: e.target.value})} />
              </div>
              <label className="flex items-center gap-3 cursor-pointer group text-left">
                <div onClick={() => setFormData({...formData, recorrente: !formData.recorrente})} className={`w-6 h-6 rounded-lg border-2 flex items-center justify-center transition-all group-hover:scale-110 ${formData.recorrente ? 'bg-[#5643ff] border-[#5643ff] shadow-[0_0_10px_rgba(86,67,255,0.4)]' : 'border-gray-600'}`}>
              ...
              <button type="submit" className="w-full bg-[#5643ff] text-white py-6 rounded-2xl font-black shadow-lg shadow-[#5643ff]/30 uppercase tracking-widest text-[11px] hover:scale-105 active:scale-95 transition-all">SALVAR REGISTRO</button>
            </form>
          </div>
        </div>
      )}
