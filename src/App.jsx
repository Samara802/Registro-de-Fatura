...
const SpreadsheetApp = () => {
  ...
  const [filtroBanco, setFiltroBanco] = useState("Todos"); // FILTRO DE BANCO RECUPERADO
    ...
    <div className={`min-h-screen p-4 md:p-10 transition-all duration-700 ${darkMode ? 'bg-[#020617] text-gray-100' : 'bg-[#f8f9ff] text-gray-900'}`}>
      ...
      <div className="max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-center gap-6 mb-12">
        ...
        <div className="flex gap-3 w-full md:w-auto">
          <button onClick={() => setIsModalOpen(true)} className="flex-1 md:flex-none bg-[#5643ff] text-white px-10 py-4 rounded-2xl font-black uppercase text-[11px] tracking-widest shadow-lg active:scale-95 transition-all">
      ...
      <div className={`max-w-7xl mx-auto rounded-[45px] shadow-2xl border ${darkMode ? 'bg-gray-900/40 border-white/5' : 'bg-white border-gray-50'}`}>
        <div className="p-10 space-y-8">
          ...
          {/* BARRA DE BUSCA E FILTROS DE BANCO (RECUPERADOS) */}
          <div className="flex flex-col xl:flex-row justify-between items-center gap-6">
            <div className={`flex items-center gap-4 px-6 py-4 w-full xl:max-w-md rounded-2xl ${darkMode ? 'bg-white/5' : 'bg-gray-50'}`}>
            ...
            <div className="flex flex-wrap justify-center gap-2">
              {["Todos", "BB", "C6", "Nubank"].map((b) => (
                <button key={b} onClick={() => setFiltroBanco(b)} className={`px-5 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all border ${filtroBanco === b ? 'bg-white/10 border-indigo-500 text-white shadow-lg shadow-indigo-500/20' : 'border-white/5 text-gray-500 hover:border-white/10'}`}>
                  {b}
                </button>
              ))}
            </div>
                    ...
                    <tr key={item.id} className={`transition-all duration-300 border-b border-white/[0.02] ${isSelected ? 'bg-[#5643ff]/10' : vencido ? 'bg-red-500/[0.04] animate-pulse' : ''} hover:bg-white/[0.02]`}>
                      ...
                      <td className="px-8 py-6 text-left">
                        <div className="flex flex-col">
                          ...
                          {vencido && <span className="text-[8px] font-black text-red-500 flex items-center gap-1 mt-1 uppercase tracking-tighter"><AlertCircle size={10}/> VENCIDO</span>}
                        </div>
                      </td>
                      <td className="px-8 py-6 text-left">
